...
const CARD_BASE = 'rounded-2xl border border-border bg-card p-4';
...
export function AnalysisView({
  ...
  return (
    <div className="mt-6 space-y-6">
      {/* Hero band: identity, price, sparkline, valuation verdict + range.
          Stacks on mobile; identity | sparkline split on wider screens. */}
      ...
      {/* Analysis cards — responsive dashboard grid (1 col → 2 → 3). Each Section
          owns its header + card; grid gap handles the spacing between them. */}
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
        {/* Editable assumptions — drives the live recompute in the hero above */}
        <Section
          title="Assumptions"
          infoKey="dcf"
          action={
        ...
        <Section title="Valuation range" infoKey="marginOfSafety">
          <div className={GRID}>
        ...
        {/* Valuation multiples */}
        <Section title="Multiples" infoKey="multiples">
          <div className={GRID}>
      ...
      {/* AI thesis (BYOK) — Claude reasons over the engine numbers above.
          Capped width: long-form prose reads better in a narrower column. */}
      <section className="max-w-3xl">
        <div className="flex items-center">
          <h2 className="text-base font-bold text-fg">AI thesis</h2>
        </div>
        <ThesisCard
          state={thesisState}
...
// --- titled section (header + card) — one grid cell ----------------------
// Bundles the heading and its card so the whole unit flows as a single item in
// the responsive dashboard grid. `flex-1` lets cards in the same row share a
// height (clean bottoms) regardless of content length.
...
function Section({
  ...
  return (
    <section className="flex flex-col">
      ...
      <div className={`mt-2 flex-1 ${CARD_BASE}`}>{children}</div>
    </section>
  );
}
...
function PeerCompsCard({
  ...
  if (loading) {
    return (
      <Section title="Peer comps" infoKey="comps">
